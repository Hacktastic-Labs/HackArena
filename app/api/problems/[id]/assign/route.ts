  ...
  try {
    ...
    if (studentAssignedMentorId) {
      ...
      if (problem.userId !== session.user.id) {
        return NextResponse.json(
          { error: "Only the author can assign a mentor" },
          { status: 403 }
        );
      }
      const mentor = await prisma.user.findFirst({
        where: { id: studentAssignedMentorId, role: "MENTOR" },
      });
      if (!mentor) {
        return NextResponse.json(
      ...
      if ((session.user as any)?.role !== "MENTOR") {
        return NextResponse.json(
          { error: "Only mentors can take this action" },
          { status: 403 }
        );
      }
      const problem = await prisma.problem.findUnique({
      ...
      if (problem.status !== "OPEN") {
        return NextResponse.json(
          { error: "Problem is not open for assignment" },
          { status: 400 }
        );
      }
    ...
    if (!assignedMentorId) {
      return NextResponse.json(
        { error: "Could not determine mentor for assignment" },
        { status: 400 }
      );
    }
